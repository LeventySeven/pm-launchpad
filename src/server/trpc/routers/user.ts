...
const formatUser = (row: UserRow, balanceMinor: number = 0) => ({
  ...
  balance: toMajorUnits(balanceMinor, VOTE_DECIMALS),
...
const buildRealizedPnlByUser = (
  ...
  sorted.forEach((trade) => {
    ...
    const grossMajor = toMajorUnits(Number(trade.collateral_gross_minor ?? 0), VOTE_DECIMALS);
...
export const userRouter = router({
  ...
  myWalletTransactions: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      return rows.map((tx) => ({
        ...
        amountMajor: toMajorUnits(Number(tx.amount_minor), VOTE_DECIMALS),
        kind: tx.kind,
  ...
  publicUserStats: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const pnlMajor = row ? toMajorUnits(Number(row.pnl_minor ?? 0), VOTE_DECIMALS) : 0;
      ...
      try {
        ...
        if (volData) {
          ...
          totalVolumeMajor = toMajorUnits(total, VOTE_DECIMALS);
        }
      } catch {
        // Best-effort
      }
  ...
  publicUserPnlSeries: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return rows.map((r) => ({
        ...
        pnlMajor: toMajorUnits(Number(r.pnl_minor ?? 0), VOTE_DECIMALS),
      }));
    }),
  ...
  publicUserTransactions: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return rows.map((r) => ({
        id: r.id,
        kind: String(r.kind),
        amountMajor: toMajorUnits(Number(r.amount_minor ?? 0), VOTE_DECIMALS),
    ...
    .query(async ({ ctx, input }) => {
      ...
      return rows.map((r, idx) => {
        ...
        return {
          ...
          avatar,
          balance: toMajorUnits(Number(r.balance_minor ?? 0), VOTE_DECIMALS),
          pnl: toMajorUnits(Number(r.pnl_minor ?? 0), VOTE_DECIMALS),
