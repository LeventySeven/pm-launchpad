...
const getQuoteSignerAccount = () => {
  const pk = (process.env.QUOTE_SIGNER_PRIVATE_KEY || "").trim();
  if (!pk) return null;
...
// Match the SQL binary search used in place_bet_tx.sql (60 iterations).
const findSharesForTargetCost = (qYes: number, qNo: number, b: number, side: "YES" | "NO", targetCost: number) => {
  if (!Number.isFinite(targetCost) || targetCost <= 0) return null;
  if (!Number.isFinite(qYes) || !Number.isFinite(qNo) || !Number.isFinite(b) || b <= 0) return null;

  const costBefore = lmsrCost(qYes, qNo, b);
  const costDelta = (shares: number) => {
    const newQYes = side === "YES" ? qYes + shares : qYes;
    const newQNo = side === "NO" ? qNo + shares : qNo;
    return lmsrCost(newQYes, newQNo, b) - costBefore;
  };

  let low = 0;
  let high = Math.max(targetCost, 0.000001);
  // Expand high until we cover the target cost.
  for (let i = 0; i < 128; i++) {
    const c = costDelta(high);
    if (Number.isFinite(c) && c >= targetCost) break;
    high *= 2;
  }

  for (let i = 0; i < 60; i++) {
    const mid = (low + high) / 2;
    const c = costDelta(mid);
    if (!Number.isFinite(c)) return null;
    if (Math.abs(c - targetCost) <= 1e-9) break;
    if (c < targetCost) low = mid;
    else high = mid;
  }
...
export const marketRouter = router({
  ...
  // ============================================================================
  // On-chain transaction preparation endpoints (Phase 4)
  // These do NOT execute DB trades. They only prepare calldata for wallet signing.
  // ============================================================================
  ...
  prepareBet: publicProcedure
    .input(
      z.object({
        marketId: z.string().uuid(),
        side: z.enum(["YES", "NO"]),
        amount: z.number().positive(), // major units (e.g., 1.25 USDC)
    ...
    .output(
      z.object({
        ...
        quoteSigner: z.string().regex(ADDRESS_RE),
        nonce: z.string(), // bigint-as-string
        marketIdBytes32: z.string().regex(/^0x[0-9a-fA-F]{64}$/),
        outcome: z.number().int(),
        amountMinor: z.number().int().positive(),
        sharesMinor: z.string(), // bigint-as-string
        sharesMajor: z.number(),
        priceBefore: z.number(),
        priceAfter: z.number(),
        quoteSignature: z.string().regex(/^0x[0-9a-fA-F]{130}$/),
        tx: z.object({
          chainId: z.number().int().positive(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const walletAddress = String((user as any).wallet_address || "");
      const chainId = Number((user as any).chain_id || 0);
      ...
      if (!ADDRESS_RE.test(vaultAddress)) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "VAULT_ADDRESS_NOT_CONFIGURED" });
      if (!ADDRESS_RE.test(tokenAddress)) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "TOKEN_ADDRESS_NOT_CONFIGURED" });

      // Load market + AMM state
      const { data: market, error: marketErr } = await supabaseService
        .from("markets")
        .select("id, state, settlement_asset_code, fee_bps, market_amm_state (b, q_yes, q_no)")
        .eq("id", input.marketId)
        .single();
      if (marketErr || !market) throw new TRPCError({ code: "NOT_FOUND", message: "Market not found" });

      const settlementAsset = String((market as any).settlement_asset_code || "");
      const state = String((market as any).state || "");
      const feeBps = Number((market as any).fee_bps ?? 0);
      const amm = (market as any).market_amm_state as { b: any; q_yes: any; q_no: any } | null;
      if (!amm) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "AMM_STATE_MISSING" });
      if (state !== "open") throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_CLOSED" });
      if (settlementAsset !== input.assetCode) {
        // Keep VCOIN markets untouched; on-chain only for USDC/USDT-marked markets.
        throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_ASSET_MISMATCH" });
      }
      // Contract currently has no fee logic; enforce 0 fees for correctness.
      if (feeBps !== 0) throw new TRPCError({ code: "BAD_REQUEST", message: "ONCHAIN_FEES_UNSUPPORTED" });

      // Asset decimals
      const { data: asset, error: assetErr } = await supabaseService
        .from("assets")
        .select("decimals, is_enabled")
        .eq("code", input.assetCode)
        .maybeSingle();
      if (assetErr || !asset) throw new TRPCError({ code: "BAD_REQUEST", message: "ASSET_NOT_FOUND" });
      if (!(asset as any).is_enabled) throw new TRPCError({ code: "BAD_REQUEST", message: "ASSET_DISABLED" });
      const decimals = Number((asset as any).decimals ?? 6);
      ...
      // feeBps == 0 => net cost == gross cost
      const targetCostMajor = amountMinor / Math.pow(10, decimals);

      const qYes = Number(amm.q_yes ?? 0);
      const qNo = Number(amm.q_no ?? 0);
      const b = Number(amm.b ?? 0);

      const sharesMajor = findSharesForTargetCost(qYes, qNo, b, input.side, targetCostMajor);
      if (!sharesMajor) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "SHARES_CALC_FAILED" });

      // Shares are represented on-chain in token-minor units (1 share == 1 token payout).
      // We scale sharesMajor (float) to 1e6-minor integer, matching USDC decimals.
      const sharesMinorBig = BigInt(Math.floor(sharesMajor * 1e6));
      if (sharesMinorBig <= 0n) throw new TRPCError({ code: "BAD_REQUEST", message: "AMOUNT_TOO_SMALL" });

      const marketIdBytes32 = uuidToBytes32(input.marketId);
      const outcome = input.side === "YES" ? 1 : 2;

      const { priceYes: priceBefore } = calculateLMSRPrices(qYes, qNo, b);
      const newQYes = input.side === "YES" ? qYes + sharesMajor : qYes;
      const newQNo = input.side === "NO" ? qNo + sharesMajor : qNo;
      const { priceYes: priceAfter } = calculateLMSRPrices(newQYes, newQNo, b);

      const deadline = Math.floor(Date.now() / 1000) + 60 * 30; // 30 min

      // Ensure we can sign quotes and the vault expects this signer.
      ...
      const account = getQuoteSignerAccount();
      if (!account) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "QUOTE_SIGNER_NOT_CONFIGURED" });
      ...
      const quoteSignerOnchain = (await (client as any).readContract({
        address: vaultAddress as `0x${string}`,
        abi: PREDICTION_MARKET_VAULT_ABI,
        functionName: "quoteSigner",
        args: [],
      })) as `0x${string}`;

      if (quoteSignerOnchain.toLowerCase() !== account.address.toLowerCase()) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "QUOTE_SIGNER_MISMATCH" });
      }
      ...
      const quoteSignature = await account.signTypedData({
        ...
        message: {
          ...
          collateral: BigInt(amountMinor),
          shares: sharesMinorBig,
          nonce,
          deadline: BigInt(deadline),
        },
      });

      // Persist deterministic on-chain market id mapping for reconciliation.
      // Best-effort: doesn't block bet prep if DB write fails.
      await supabaseService
        .from("markets")
        .update({ onchain_market_id: marketIdBytes32 } as any)
        .eq("id", input.marketId)
        .is("onchain_market_id", null);

      await supabaseService.from("market_onchain_map").upsert(
        {
          market_id: input.marketId,
          chain_id: chainId,
          vault_address: vaultAddress,
          onchain_market_id: marketIdBytes32,
        } as any,
        { onConflict: "market_id,chain_id" }
      );
      ...
      return {
        ...
        vaultAddress,
        tokenAddress,
        quoteSigner: quoteSignerOnchain,
        nonce: nonce.toString(),
        marketIdBytes32,
        outcome,
        amountMinor,
        sharesMinor: sharesMinorBig.toString(),
        sharesMajor,
        priceBefore,
        priceAfter,
        quoteSignature,
        tx: { chainId, to: vaultAddress, data, value: "0" },
        deadline,
      };
    }),
  ...
  prepareSell: publicProcedure
    .input(
      z.object({
        marketId: z.string().uuid(),
        side: z.enum(["YES", "NO"]),
        shares: z.number().positive(), // major shares (same units as DB positions)
        assetCode: z.enum(["USDC", "USDT"]),
      })
    )
    .output(
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!amm) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "AMM_STATE_MISSING" });
      if (state !== "open") throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_CLOSED" });
      if (settlementAsset !== input.assetCode) throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_ASSET_MISMATCH" });
      if (feeBps !== 0) throw new TRPCError({ code: "BAD_REQUEST", message: "ONCHAIN_FEES_UNSUPPORTED" });

      // Validate user has enough off-chain shares (UX guard; on-chain is source of truth later)
      const { data: pos, error: posErr } = await supabaseService
        .from("positions")
        .select("shares")
        .eq("user_id", authUser.id)
        .eq("market_id", input.marketId)
        .eq("outcome", input.side)
        .maybeSingle();
      if (posErr) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: posErr.message });
      const held = pos ? Number((pos as any).shares ?? 0) : 0;
      if (held <= 0) throw new TRPCError({ code: "BAD_REQUEST", message: "NO_POSITION" });
      if (input.shares > held + 1e-12) throw new TRPCError({ code: "BAD_REQUEST", message: "INSUFFICIENT_SHARES" });
      ...
      // Market pool safety
      const qSide = input.side === "YES" ? qYes : qNo;
      if (input.shares > qSide + 1e-12) throw new TRPCError({ code: "BAD_REQUEST", message: "SHARES_TOO_LARGE" });

      const proceedsMajor = calculateSellProceeds(qYes, qNo, b, input.side, input.shares);
      if (!Number.isFinite(proceedsMajor) || proceedsMajor < 0) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "PAYOUT_CALC_FAILED" });
      const payoutMinor = Math.floor(proceedsMajor * Math.pow(10, decimals));

      const sharesMinorBig = BigInt(Math.floor(input.shares * 1e6));
      if (sharesMinorBig <= 0n) throw new TRPCError({ code: "BAD_REQUEST", message: "INVALID_SHARES" });

      const marketIdBytes32 = uuidToBytes32(input.marketId);
      const outcome = input.side === "YES" ? 1 : 2;

      const { priceYes: priceBefore } = calculateLMSRPrices(qYes, qNo, b);
      const newQYes = input.side === "YES" ? qYes - input.shares : qYes;
      const newQNo = input.side === "NO" ? qNo - input.shares : qNo;
      const { priceYes: priceAfter } = calculateLMSRPrices(newQYes, newQNo, b);
      ...
      const quoteSignature = await account.signTypedData({
        ...
        message: {
          ...
          token: tokenAddress as `0x${string}`,
          shares: sharesMinorBig,
          payout: BigInt(Math.max(0, payoutMinor)),
          nonce,
          deadline: BigInt(deadline),
        },
      });
      ...
      return {
        ...
        outcome,
        sharesMinor: sharesMinorBig.toString(),
        payoutMinor: Math.max(0, payoutMinor),
        payoutMajor: proceedsMajor,
        priceBefore,
        priceAfter,
        quoteSignature,
        tx: { chainId, to: vaultAddress, data, value: "0" },
  ...
  prepareClaim: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const state = String((market as any).state || "");
      const outcome = (market as any).resolve_outcome as "YES" | "NO" | null;
      const settlementAsset = String((market as any).settlement_asset_code || "");
      if (settlementAsset !== input.assetCode) throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_ASSET_MISMATCH" });
      if (state !== "resolved" || !outcome) throw new TRPCError({ code: "BAD_REQUEST", message: "MARKET_NOT_RESOLVED" });

      const marketIdBytes32 = uuidToBytes32(input.marketId);
      const data = encodeFunctionData({
        abi: PREDICTION_MARKET_VAULT_ABI,
        functionName: "claimWinnings",
        args: [marketIdBytes32, tokenAddress as `0x${string}`],
      });
