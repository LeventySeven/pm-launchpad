...
import type { MarketApiRow as SSRMarketApiRow, MarketCategoryRow as SSRMarketCategoryRow } from "@/src/server/markets/pageData";
...
// ---------------------------------------------------------------------------
// Warm Cache — localStorage with TTL for instant revisits
// ---------------------------------------------------------------------------
const WARM_CACHE_KEY = "catalog_bootstrap_v1";
const WARM_CACHE_TTL_MS = 90_000; // 90 seconds

type WarmCachePayload = {
  fetchedAt: number;
  markets: SSRMarketApiRow[];
  categories: SSRMarketCategoryRow[];
};

function readWarmCatalogBootstrap(): WarmCachePayload | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(WARM_CACHE_KEY);
    if (!raw) return null;
    const parsed: WarmCachePayload = JSON.parse(raw);
    if (Date.now() - parsed.fetchedAt > WARM_CACHE_TTL_MS) return null;
    if (!Array.isArray(parsed.markets) || !Array.isArray(parsed.categories)) return null;
    return parsed;
  } catch {
    return null;
  }
...
function writeWarmCatalogCache(markets: SSRMarketApiRow[], categories: SSRMarketCategoryRow[]) {
  if (typeof window === "undefined") return;
  try {
    const payload: WarmCachePayload = {
      fetchedAt: Date.now(),
      markets,
      categories,
    };
    localStorage.setItem(WARM_CACHE_KEY, JSON.stringify(payload));
  } catch {
...
export type HomePageClientProps = {
  /** SSR-bootstrapped markets (baked into HTML). Empty array = no SSR data. */
  initialMarkets?: SSRMarketApiRow[];
  /** SSR-bootstrapped categories. */
  initialCategories?: SSRMarketCategoryRow[];
...
export default function HomePageClient({
  ...
  const bootstrapData = useMemo(() => {
    // Layer 0: SSR props (best — data is in the HTML)
    if (initialMarkets && initialMarkets.length > 0) {
      return { markets: initialMarkets, categories: initialCategories ?? [], source: "ssr" as const };
    }
    // Layer 1: Warm cache from localStorage (instant revisits)
    const warm = readWarmCatalogBootstrap();
    if (warm) {
      return { markets: warm.markets, categories: warm.categories, source: "warm-cache" as const };
    }
    // Layer 2: Empty — client will fetch
  ...
  // SSR bootstrap: if we have pre-fetched markets, map them immediately so
  // the first paint already contains market cards — no loading spinner.
  const [markets, setMarkets] = useState<Market[]>(() => {
    if (bootstrapData.markets.length > 0) {
      const langVal = typeof window !== "undefined"
        ? (localStorage.getItem("lang") === "RU" ? "RU" : "EN") as "RU" | "EN"
        : "EN";
      return bootstrapData.markets.map((m) => mapMarketApiToMarket(m as MarketApiRow, langVal));
    }
    return [];
  });
  const [loadingMarkets, setLoadingMarkets] = useState(bootstrapData.source === "none");
  /** Track whether the initial SSR/warm-cache data has been superseded by a fresh fetch. */
  const hasLoadedFreshDataRef = useRef(bootstrapData.source !== "none");
  /** Ref to track whether we have markets (avoids stale closure in loadMarkets). */
  const hasMarketsRef = useRef(bootstrapData.markets.length > 0);
  ...
  // Deep link: open a market by URL (?marketId=...) or from SSR props.
  useEffect(() => {
    ...
    // SSR-provided market ID takes precedence for /market/[marketId] routes
    const marketId = marketIdFromUrl ?? marketIdFromStartParam ?? initialMarketId ?? null;
    if (!marketId) return;
  ...
  // Stale-while-revalidate caches for market detail data
  const CANDLE_CACHE_TTL_MS = 30_000; // 30 seconds
  const marketCandlesCacheRef = useRef<Map<string, { candles: PriceCandle[]; cachedAt: number }>>(new Map());
  const marketTradesCacheRef = useRef<Map<string, { trades: PublicTrade[]; cachedAt: number }>>(new Map());
  ...
  const [marketCategories, setMarketCategories] = useState<MarketCategoryStrict[]>(
    () => bootstrapData.categories.map((c) => ({
      id: c.id,
      labelRu: c.labelRu,
      labelEn: c.labelEn,
    }))
  ...
  const loadMarkets = useCallback(async () => {
    // Don't show loading state if we have bootstrap data (silent refresh)
    if (!hasMarketsRef.current) {
    ...
    try {
      ...
      setMarkets(mapped);
      hasMarketsRef.current = mapped.length > 0;

      // Write warm cache so next visit is instant
      writeWarmCatalogCache(
        (response ?? []).map((m) => ({
          id: m.id, titleRu: m.titleRu, titleEn: m.titleEn,
          description: m.description, source: m.source, imageUrl: m.imageUrl,
          state: m.state, createdAt: m.createdAt, closesAt: m.closesAt,
          expiresAt: m.expiresAt, marketType: m.marketType,
          resolvedOutcomeId: m.resolvedOutcomeId, outcomes: m.outcomes,
          outcome: m.outcome, createdBy: m.createdBy,
          categoryId: m.categoryId, categoryLabelRu: m.categoryLabelRu,
          categoryLabelEn: m.categoryLabelEn, settlementAsset: m.settlementAsset,
          feeBps: m.feeBps, liquidityB: m.liquidityB,
          priceYes: m.priceYes, priceNo: m.priceNo, volume: m.volume,
          chance: m.chance, creatorName: m.creatorName,
          creatorAvatarUrl: m.creatorAvatarUrl,
        })) as SSRMarketApiRow[],
        marketCategories.map((c) => ({ id: c.id, labelRu: c.labelRu, labelEn: c.labelEn })),
      );
    } catch (err) {
      ...
      // Don't clear markets if we have bootstrap data — keep showing stale
      if (!hasMarketsRef.current) {
        setMarkets([]);
      }
    } finally {
  ...
  useEffect(() => {
    if (bootstrapData.source !== "none" && !hasLoadedFreshDataRef.current) {
      // We have SSR or warm-cache data — skip the initial fetch, but schedule
      // a background refresh so the user gets fresh data silently.
      hasLoadedFreshDataRef.current = true;
      const timer = setTimeout(() => void loadMarkets(), 200);
    ...
    return undefined;
  }, [loadMarkets, loadMarketCategories, marketCategories.length, loadingMarketCategories, bootstrapData.source]);
  ...
  // ---------------------------------------------------------------------------
  // Client-side virtual pagination with IntersectionObserver
  // ---------------------------------------------------------------------------
  ...
  }, [activeCategoryId, searchQuery, catalogSort, catalogStatus, catalogTimeFilter]);

  const visibleCatalogMarkets = useMemo(
    () => catalogMarkets.slice(0, catalogVisibleCount),
    [catalogMarkets, catalogVisibleCount],
  );
  ...
  useEffect(() => {
    ...
    // Stale-while-revalidate: show cached data instantly, then fetch fresh.
    // Show data even if slightly stale (within 5x TTL) — fetch will replace it.
    const SWR_MAX_STALE_MS = CANDLE_CACHE_TTL_MS * 5; // 2.5 minutes max stale
    const cachedCandles = marketCandlesCacheRef.current.get(selectedMarketId);
    const cachedTrades = marketTradesCacheRef.current.get(selectedMarketId);
    const candleAge = cachedCandles ? Date.now() - cachedCandles.cachedAt : Infinity;
    const tradeAge = cachedTrades ? Date.now() - cachedTrades.cachedAt : Infinity;
    if (cachedCandles && cachedCandles.candles.length > 0 && candleAge < SWR_MAX_STALE_MS) {
      setMarketCandles(cachedCandles.candles);
    }
    if (cachedTrades && cachedTrades.trades.length > 0 && tradeAge < SWR_MAX_STALE_MS) {
      setMarketPublicTrades(cachedTrades.trades);
    }
    ...
    const fetchInsights = async () => {
      ...
      const hasCachedData = (cachedCandles?.candles?.length ?? 0) > 0;
      setMarketInsightsLoading(!hasCachedData);
      setMarketInsightsError(null);
      setMarketCommentsError(null);
      setMarketActivityError(null);
      try {
        ...
        // Candles (chart) — write to stale-while-revalidate cache
        if (candlesRes.status === "fulfilled") {
          ...
          marketCandlesCacheRef.current.set(selectedMarketId, { candles, cachedAt: Date.now() });
        } else {
        ...
        if (tradesRes.status === "fulfilled") {
          ...
          marketTradesCacheRef.current.set(selectedMarketId, { trades, cachedAt: Date.now() });
        } else {
                      ...
                      {loadingMarkets ? (
                        ...
                        <>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 pb-8">
                            {visibleCatalogMarkets.map((market) => (
                              <MarketCard
                                key={market.id}
                                market={market}
                          ...
                          {/* Invisible sentinel for IntersectionObserver — triggers next page load */}
