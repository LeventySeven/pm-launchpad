...
-- Fix leaderboard ordering stability:
-- - `rank()` only considers pnl_minor, so ties (e.g. many users at 0 pnl) all get rank=1.
-- - The API was ordering by `rank` only, producing effectively random ordering for ties.
